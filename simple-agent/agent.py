...
class TimeAgent(BaseAgent):
    """Agent specialized for time-related queries."""
    ...
    def __init__(
        ...
        # Define the system prompt for time assistant
        system_prompt = """You are a helpful time assistant with access to the get_current_time function.

When the user FIRST asks for the current time, respond with this JSON:
{"name": "get_current_time", "arguments": {}}

When you receive tool results (messages starting with "Tool"), use those results to answer the user naturally.
Always return the time in 24-hour time format. It is CRITICAL the time always be returned in this format.

Example flow:
User: "What time is it?"
You: {"name": "get_current_time", "arguments": {}}
Tool result: "06:30 AM"
You: "The current time is 06:30"

Important: You do NOT know the time without calling the function first."""
    ...
    def _register_tools(self):
        ...
        @self.agent.tool
        def get_current_time_tool(ctx: RunContext) -> str:
            """
            Get the current system time.
        ...
        self.tool_mapping['get_current_time_tool'] = get_current_time
